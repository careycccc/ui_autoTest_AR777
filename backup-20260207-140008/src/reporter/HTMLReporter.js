...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-nav { background: white; border-radius: 12px; padding: 16px; margin-bottom: 20px; display: flex; flex-wrap: wrap; gap: 10px; }
...
<body>
  <div class="container">
    ...
    <div class="stats">
      <div class="stat-card"><div class="stat-value">${total}</div><div class="stat-label">总测试数</div></div>
      <div class="stat-card stat-passed"><div class="stat-value">${passed}</div><div class="stat-label">✅ 通过</div></div>
    ...
    <div class="page-nav">
      ...
      ${allPageRecords.map((page, i) => `<button class="page-nav-btn ${i === 0 ? 'active' : ''}" data-index="${i}">${page.name || '页面 ' + (i + 1)}${page.apiErrors?.length > 0 ? '<span class="error-badge">' + page.apiErrors.length + '</span>' : ''}</button>`).join('')}
    </div>
      ...
      <div id="page-${index}" class="page-section ${index === 0 ? 'active' : ''}">
        <div class="page-header">
          ...
          <div class="page-meta">
            ...
            <span>🕐 ${page.startTime ? new Date(page.startTime).toLocaleTimeString('zh-CN') : 'N/A'}</span>
      ...
      issue.causes.forEach(c => {
        ...
        if (c.resources && c.resources.length > 0) {
          ...
          c.resources.forEach(r => {
            html += '<tr><td class="resource-url" title="' + (r.url || '') + '">' + (r.url || r.name || '-') + '</td><td>' + (r.type || '-') + '</td><td>' + (r.duration || r.time || r.ttfb || '-') + '</td><td>' + (r.size || '-') + '</td></tr>';
          });
  ...
  renderApiRow(req, pageIndex, reqIndex) {
    const status = req.response?.status || (req.status === 'failed' ? 'ERR' : '-');
